...
public class NdfdXMLLocator extends org.apache.axis.client.Service implements weather.NdfdXML {
 ...
 * The Unit will default to U.S. Standard (english) unless Metric is
 *             chosen by client. The client also needs to provide the
 * format that is desired.
 ...
 *             the upper right corners of a rectangular subgrid.  The
 * function can also take a integer
 ...
 * For the LatLonListLine, The inputs are the same as the function NDFDgen
 * except the latitude and longitude pair is
 *             replaced by two latitude and longitude pairs, one for
 ...
 *             are relaced by a zip code for the 50 United States and
 * Puerto Rico.
 *             For the CornerPoints function, the service requires a
 * valid NDFD grid name.  The function returns a
 *             list of four latitude and longitude pairs, one for each
 * corner of the NDFD grid.  The function
 *             also returns the minimum resolution required to return
 * the entire grid below the maximum points
 *             threshold.
 *             For the GmlLatLonList function, the service requires a
 * list of latitude and longitude pairs, the time (UTC) the user
 *             wants data for, the GML feature type and the array of
 * boolean values corresponding to which NDFD values are desired.
 *             For the GmlTimeSeries function, the service requires a
 * list of latitude and longitude pairs, the start and end time (UTC)
 * the user
 ...
 * large main cities.  Level 2 represents progressively smaller cities
 * or large cities that are close to another even larger city.  Levels
 ...
 *             returns a list of latitude and longitude values along
 * with a seperate list of city name for those point.
 */
    ...
    public weather.NdfdXMLPortType getndfdXMLPort() throws javax.xml.rpc.ServiceException {
    ...
    public weather.NdfdXMLPortType getndfdXMLPort(java.net.URL portAddress) throws javax.xml.rpc.ServiceException {
        try {
            weather.NdfdXMLBindingStub _stub = new weather.NdfdXMLBindingStub(portAddress, this);
    ...
    public java.rmi.Remote getPort(Class serviceEndpointInterface) throws javax.xml.rpc.ServiceException {
        try {
            if (weather.NdfdXMLPortType.class.isAssignableFrom(serviceEndpointInterface)) {
                weather.NdfdXMLBindingStub _stub = new weather.NdfdXMLBindingStub(new java.net.URL(ndfdXMLPort_address), this);
                _stub.setPortName(getndfdXMLPortWSDDServiceName());
                return _stub;
            }
        }
